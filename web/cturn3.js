// This is Javascript
//
// stance narrows coming out of the fall line
turnClass = class {
        constructor() {
                this.max_width = 8 // Pixels

                this.center_width = 1 // Pixels
                this.center_color = 'black'

                this.ski_color = 'purple' // Rossi 3G
                this.ski_length = 100 // Pixels
                this.ski_width = 1 // Pixels
                this.ski_mod = 30 // Pixels, how often to draw

                this.initial_position = [200, 0]
        }

        // Generate and return the path
        getPath() {
		// starting point
		var start = [
			[0,45,'black','black',50,50,20,20,0,55,0,55]
		];

		var left_footer = [
			// left-footer top
			[100, -30,'grey','blue',20,80,20,20,0,55,0,55],

			// left-footer into the fall line (specify 90/10 at the fall line)
			[50, -15, 'gray', 'black',10,90,20,20,0,55,0,55],

			// out of the fall line
			[50, -15, 'gray', 'black',20,80,10,20,0,55,0,55],

			// end of the turn
			[100, -30, 'gray', 'blue',50,50,10,20,0,55,0,55],
		];

		var right_footer = [
			// right-footer top
			[100, 30,'blue','grey',80,20,20,20,0,55,0,55],

			// into the fall line (specify 90/10 at the fall line)
			[50, 15, 'black', 'grey',90,10,20,20,0,55,0,55],

			// out of the fall line
			[50, 15, 'black', 'grey',80,30,20,10,0,55,0,55],

			// end of the turn
			[100, 30, 'blue', 'grey',50,50,20,10,0,55,0,55],
		];

		// Path is required
		return start.concat(left_footer).concat(right_footer);
	}
}
